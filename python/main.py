...
while(cap.isOpened()):   
    ...
    if config['text_overlay']:
        ...
        str_on_frame = "Frame: %d/%d" % (video_cur_frame, video_info['num_of_frames'])
        cv2.putText(frame_out, str_on_frame, (5,30), cv2.FONT_HERSHEY_COMPLEX,
                    0.7, (0,128,255), 2, cv2.LINE_AA)
        str_on_frame = "Titik Tersedia: %d Titik Tidak Tersedia: %d" % (spot, occupied)
        cv2.putText(frame_out, str_on_frame, (5,90), cv2.FONT_HERSHEY_COMPLEX,
                            0.7, (0,128,255), 2, cv2.LINE_AA)
